...
int objloader::load(const char* filename)
{
	...
	{
		if(coord[i]->c_str()[0]=='#') //if it's a comment
			continue;   //we don't have to do anything with it
		else if(coord[i]->c_str()[0]=='v' && coord[i]->c_str()[1]==' ') //if a vertex
		{
		...
		}else if(coord[i]->c_str()[0]=='v' && coord[i]->c_str()[1]=='n')    //if it's a normal vector
		{
			float tmpx,tmpy,tmpz;
		...
		}else if(coord[i]->c_str()[0]=='u' && coord[i]->c_str()[1]=='s' && coord[i]->c_str()[2]=='e') //use material material_name
		{
			char tmp[200];
		...
		}else if(coord[i]->c_str()[0]=='m' && coord[i]->c_str()[1]=='t' && coord[i]->c_str()[2]=='l' && coord[i]->c_str()[3]=='l')  //material library, a file, which contain
			//all of the materials
		{
			...
			for(int i=0;i<tmp.size();i++) //go through all lines of the mtllib file
			{
				...
				}else if(tmp[i].c_str()[0]=='N' && tmp[i].c_str()[1]=='s')  //the shininess
				{
					sscanf(tmp[i].c_str(),"Ns %f",&ns);
					ismat=true;
				}else if(tmp[i].c_str()[0]=='K' && tmp[i].c_str()[1]=='a')  //the ambient
				{
					sscanf(tmp[i].c_str(),"Ka %f %f %f",&amb[0],&amb[1],&amb[2]);
					ismat=true;
				}else if(tmp[i].c_str()[0]=='K' && tmp[i].c_str()[1]=='d')  //the diffuse
				{
					sscanf(tmp[i].c_str(),"Kd %f %f %f",&dif[0],&dif[1],&dif[2]);
					ismat=true;
				}else if(tmp[i].c_str()[0]=='K' && tmp[i].c_str()[1]=='s')  //the specular
				{
					sscanf(tmp[i].c_str(),"Ks %f %f %f",&spec[0],&spec[1],&spec[2]);
					ismat=true;
				}else if(tmp[i].c_str()[0]=='N' && tmp[i].c_str()[1]=='i')  //the I don't know what is this
				{
					sscanf(tmp[i].c_str(),"Ni %f",&ni);
					ismat=true;
				}else if(tmp[i].c_str()[0]=='d' && tmp[i].c_str()[1]==' ')  //the alpha
				{
					sscanf(tmp[i].c_str(),"d %f",&alpha);
					ismat=true;
				}else if(tmp[i].c_str()[0]=='i' && tmp[i].c_str()[1]=='l')  //the illum (don't ask)
				{
					sscanf(tmp[i].c_str(),"illum %d",&illum);
					ismat=true;
				}else if(tmp[i].c_str()[0]=='m' && tmp[i].c_str()[1]=='a')  //and the texture
				{
